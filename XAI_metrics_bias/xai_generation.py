...
def main():
    ...
    # Get dataset
    dataset, n_output = get_dataset(args.dataset_name, f"../data" ,args.percent, dataset_split = "test")
    # Get model
    model = get_model(args.model, dataset=args.dataset_name)
    model = model.eval()
    ...
    if args.npz_checkpoint:
        ...
        for X, attr, _ in tqdm(val_loader, total=len(val_loader), desc = "Generating saliency maps"):
    ...
    if not args.skip_metrics:
        ...
        for (X, attr, _), A in tqdm(xai_loader, desc="Computing metrics"):
            # device
            if args.gpu:
                device = "cuda"
            else:
            ...
            with torch.no_grad():
                y = torch.argmax(model(X.to(device)), axis = 1)
            ...
            conflict_align.append((attr[:,0] == attr[:,1]).int().cpu().numpy())
        ...
        file_name_conflict_align = "csv/align_conflict.csv"
        if not path.exists(file_name_conflict_align):
            pd.DataFrame({"bias": conflict_align}).to_csv(file_name_conflict_align)
